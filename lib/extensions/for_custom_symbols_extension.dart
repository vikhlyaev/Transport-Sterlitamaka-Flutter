...
extension forCustomSymbols on MapboxMapController {
  Future<List<StationSymbol>> addStationSymbols(List<StationSymbolOptions> options, [List<Map>? data]) async {
    final symbols = [
      for (var i = 0; i < options.length; i++)
        StationSymbol(getRandomString(), SymbolOptions.defaultOptions.copyWith(options[i]), options[i].id,
            options[i].name, options[i].isFavorite, data?[i])
    ];
  ...
  Future<List<TrackSymbol>> addTrackSymbols(List<TrackSymbolOptions> options, [List<Map>? data]) async {
    final symbols = [
      for (var i = 0; i < options.length; i++)
        TrackSymbol(getRandomString(), SymbolOptions.defaultOptions.copyWith(options[i]), options[i].id,
            options[i].route, data?[i])
    ];
    await symbolManager!.addAll(symbols);
